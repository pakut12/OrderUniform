<%-- 
    ...
    Author     : pakutsing
--%>
<%@page import="com.pg.lib.service.TransactionDepartmentService"%>
...
<html>
    ...
    <body>
        ...
        <script>
            ...
            List<OUTransactionDepartmentDetail> detail = s_trancustomer.getDetailTransactionByDocumentId(doc_id);
            int sum = 0;
            ...
            for (int i = 0; i <= detail.size() - 1; i++) {
                sum += Integer.valueOf(detail.get(i).getMaterialquantity().toString());
                if (id != detail.get(i).getDepartID()) {
                                %>
                                                        [ 
                                                            {
                                                                image: textToBase64Barcode('<%=doc_id%>/<%=detail.get(i).getDepartID()%>'),
                                                                width: 140,
                                                                height: 25,
                                                            ...
                                                            {
                                                                border: [false, false, false, false], 
                                                                text: 'รหัสพนักงาน <%=detail.get(i).getAgency()%> \n ชื่อ-นามสกุล : <%=detail.get(i).getDivision()%> \n เเผนก : <%=detail.get(i).getDepartmentname()%>', 
                                                                alignment:'center' ,
                                                                colSpan: 5
                                                            }
                ...
                %>
                                        ...
                                        [ 
                                            ...
                                            '<%=detail.get(i).getMaterialdesc()%>',
                                            '<%=detail.get(i).getMaterialname()%>',
                                            '<%=detail.get(i).getMaterialcolor()%>', 
                                            '<%=detail.get(i).getMaterialsize()%>', 
                                            '<%=detail.get(i).getMaterialfullname()%>' , 
                                            {
                                            ...
                                            '<%=detail.get(i).getMaterialquantity()%>'
                                                            
                                        ],
                    
                    
         <%
                id = detail.get(i).getDepartID();
